...
func AvailablePlasma(archive store.Archive, account store.Account) (uint64, error) {
	address := *account.Address()
	committed, err := archive.GetChainPlasma(address)
	if err != nil {
		return 0, err
	}
	fused, err := archive.GetStakeBeneficialAmount(address)
	if err != nil {
		return 0, err
	}
